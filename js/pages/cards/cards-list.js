 ...
 * - card-detail.js（window.generateDetailHtml or window.CardDetailTemplate.generate）
 */
...
// ✅ 詳細テンプレは card-detail.js を唯一の正にする
//  - 図鑑 / 所持率：card-detail.js が window.generateDetailHtml を公開している前提
...
const generateDetailHtml = function (card) {
  ...
  console.warn('[card-list] window.generateDetailHtml が未定義です。card-detail.js の読み込み順を確認してください。');

  // 最低限の保険（※基本はここに来ないのが正）
  const cd = window.normCd5 ? window.normCd5(card.cd) : String(card.cd ?? '').padStart(5, '0');
  ...
  const escapeAttr = value => String(value ?? '')
    ...
    .replace(/>/g, '&gt;')
    .replace(/"/g, '&quot;');
  const cvHtml = card.CV
    ? `<div class="card-meta card-cv"><span class="card-cv-label">CV</span>: <span class="card-cv-name">${card.CV || ''}</span>${card.cv_kana ? ` <span class="card-cv-kana">（${card.cv_kana}）</span>` : ''}<button type="button" class="card-cv-search-btn" data-cv="${escapeAttr(card.CV || '')}" aria-label="${escapeAttr(card.CV || '')}で検索" title="このCVで検索">🔎</button></div>`
    : '';
  return `
