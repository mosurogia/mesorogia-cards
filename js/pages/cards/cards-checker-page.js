...
(function () {
  ...
  window.selectPack = function selectPack(_value) { /* no-op */ };
...
window.jumpToPack = function jumpToPack(packSlug) {
  ...
  function getPackJumpTabs_() {
    return document.getElementById('pack-jump-tabs');
  }

  function buildPackJumpTabs_({ force = false } = {}) {
    const tabsRoot = getPackJumpTabs_();
  ...
  function initPackJumpTabsWithRetry_() {
    let tries = 0;
    const tick = () => {
      tries++;
      const ok = buildPackJumpTabs_();
      if (ok) {
        // ✅ タブ生成が完了した後に、もう一度DOM順を整える
        swapPackJumpTabAndSelectOrder_();
        return;
      }
  ...
  window.rebuildPackJumpTabs = function rebuildPackJumpTabs() {
    return buildPackJumpTabs_({ force: true });
  };

  function rebuildPackJumpTabsWithRetry_() {
    let tries = 0;
    const tick = () => {
      tries++;
      const ok = buildPackJumpTabs_({ force: true });
      if (ok) {
        swapPackJumpTabAndSelectOrder_();
        return;
      }
  ...
  function swapPackJumpTabAndSelectOrder_() {
    ...
    const sel =
      ...
      document.getElementById('pack-selector') ||
...
function getOverallFromSummary_() {
  ...
  function ensureSidebarMeter_(hostEl) {
    ...
    if (!box) {
      ...
      const action = hostEl.querySelector('.pack-summary-detail-toggle');
      if (action) hostEl.insertBefore(box, action);
  ...
  function ensurePackTitleMeters_() {
    const sections = Array.from(document.querySelectorAll('#packs-root .pack-section[id^="pack-"]'));
    sections.forEach(sec => {
      if (sec.querySelector('.pack-inline-meters')) return;

      const h3 = sec.querySelector('h3');
      if (!h3) return;

      const box = document.createElement('div');
      box.className = 'pack-inline-meters';
  ...
  function syncAllMeters_() {
    ...
    // ---- トップ：全カードゲージ ----
    const topSummary = document.getElementById('top-pack-summary');
    if (topSummary) {
      topSummary.innerHTML = overall
    ...
    items.forEach(item => {
      ...
      const targetBox = document.querySelector(`#pack-${CSS.escape(slug)} .pack-inline-meters`);
      if (targetBox) {
        targetBox.hidden = shouldHide;
        targetBox.innerHTML = shouldHide ? '' : renderMetersHtml_(p);
      }
    });
  }
...
top = clamp_(top, pad, window.innerHeight - h - pad);
  ...
  function findCardElFromEvent_(ev) {
    ...
    if (t.closest('.zoom-btn')) return null;
  ...
  // ---- Mobile: タップ時はカード名ツールチップを出さない ----
  document.addEventListener('touchstart', () => {
    hideTooltip_();
  }, { passive: true });

  document.addEventListener('touchmove', hideTooltip_, { passive: true });
  document.addEventListener('touchend', hideTooltip_, { passive: true });
  document.addEventListener('touchcancel', hideTooltip_, { passive: true });
