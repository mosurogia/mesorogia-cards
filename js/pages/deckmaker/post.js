...
使ってみた全体の印象、成果や気づきなど。`,
  ...
  function openNoteFull_(){
    ...
    if (side){
      ...
      for (const [cd, n] of entries){
        ...
        const cd5 = String(cd ?? '').trim().padStart(5,'0').slice(0,5);
        img.alt = '';
        img.loading = 'lazy';
  ...
  const CardNotes = (() => {
    ...
    function currentDeckUniqueCds() {
      ...
      const list = Object.entries(d)
        .filter(([, n]) => (n | 0) > 0)
        .map(([cd]) => String(cd).padStart(5, '0').slice(0, 5));
      // unique
  ...
  // - 外部連携：campaign.js から read/write できる互換APIを提供
  // =====================================================
  const SelectTags = (() => {
    ...
    function getDeckCategoryTags_() {
      ...
      for (const [cd, n] of Object.entries(d)) {
        if (!(n | 0)) continue;
        const c = map[cd] || map[String(cd).padStart(5,'0')] || null;
    ...
    function formatLabel_(label) {
      // 重要：XSS対策（既存の escapeHtml_ / escapeHtml を優先して呼び出す）
      const esc = (window.escapeHtml_ || window.escapeHtml || ((s)=>String(s ?? '')));
    ...
    // campaign.js 連携用（命名互換）
    // ✅ campaign.js は Set 前提で .has() を呼ぶので、ここは Set を返す
    ...
    // campaign.js より後に読み込まれた場合でも、post.js（SelectTags）を本命にする
...
// 旧: page2 の note-side-list クリックで cardOp を開く仕組みの受け皿
window.openCardOpModal = window.openCardOpModal || function(cardId){
  document.dispatchEvent(new CustomEvent('open-cardop', {
    detail: { cardId: String(cardId || '') }
  }));
};
